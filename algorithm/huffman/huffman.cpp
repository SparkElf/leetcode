...
#define ch_loop(ch,in) for(char ch=in.get();ch!=EOF;ch=in.get())
    ...
    void build_tree(vector<Node>& nodes) {
        ...
        auto q = priority_queue<Node, vector<Node>, decltype(cmp)>(nodes.begin(), nodes.end());//https://iq.opengenus.org/initialize-priority-queue-in-cpp/
    ...
    struct Compressor {
        ...
        auto toFile(filesystem::path&& p)->Compressor& {
            ...
            for (auto iter = codingTable.begin();iter != codingTable.end();iter++) { file << iter->first << " " << iter->second.first << endl; }
            ch_loop(ch,src)
    ...
    auto print(unordered_map<char, pair<int, string>>& codingTable, filesystem::path&& filepath)->void {
        ...
        for (auto iter = codingTable.begin();iter != codingTable.end();iter++) { file << iter->first << " " << iter->second.second << endl; }
    }
